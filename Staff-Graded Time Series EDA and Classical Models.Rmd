...
for (i in 1:(nrow(temps) - last_real_row - 11)) {
  ...
  forecast_row <- c(as.Date(temps$Month[last_real_row+12]), as.numeric(next_year_forecast))
...
colnames(forecasted_temps) <- c('Month', 'ForecastedTemp')
...
joined_temps <- left_join(temps, forecasted_temps, by='Month')
